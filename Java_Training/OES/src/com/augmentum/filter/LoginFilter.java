...
@WebFilter(urlPatterns = {"*.action"}, initParams = {@WebInitParam(name="noNeedLoginPages",
value="login.action")})
public class LoginFilter extends HttpFilter{

    private String noNeedLoginPages = "login.action";
    @Override
    protected void init() {
    ...
    public void doFilter(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        String requestedUri = URI.substring(URI.lastIndexOf("/") + 1);
        ...
        if (noNeedLoginPages.equals(requestedUri)) {
            ...
            if (user == null){
                if (StringUtil.isEquals("GET", request.getMethod().toUpperCase())) {
                    ...
                    response.sendRedirect(request.getContextPath() + "/" + noNeedLoginPages +
                            "?go=" + requestedUri);
                } else {
                    response.sendRedirect(request.getContextPath() + "/" + noNeedLoginPages);
                }
